...
void SmoothScroll::wheelEvent(QWheelEvent *event)
{
    ...
    #else
    m_lastWheelEvent.reset(new QWheelEvent(event->position(), event->globalPosition(), event->pixelDelta(), event->angleDelta(), event->buttons(), event->modifiers(), Qt::ScrollUpdate, false));
    #endif
...
void SmoothScroll::smoothMove()
{
    ...
    QWheelEvent e(m_lastWheelEvent->position(), m_lastWheelEvent->globalPosition(), QPoint(), p, m_lastWheelEvent->buttons(), Qt::NoModifier, Qt::ScrollUpdate, false);
