...
QListWidgetItem *RoundMenu::createActionItem(QAction *const action, QAction *const before)
{
    ...
    if (!hasItemIcon()) {
        w = 28 + m_view->fontMetrics().horizontalAdvance(action->text());
    } else {
        // add a blank character to increase space between icon and text
        item->setText(" " + item->text());
        w = 60 + m_view->fontMetrics().horizontalAdvance(item->text());
    }
...
QPair<QListWidgetItem *, SubMenuItemWidget *> RoundMenu::createSubMenuItem(RoundMenu *const menu)
{
    ...
    int w                 = 0;
    if (!hasItemIcon()) {
        w = 48 + m_view->fontMetrics().horizontalAdvance(menu->title());
    } else {
        // add a blank character to increase space between icon and text
        item->setText(" " + item->text());
        w = 60 + m_view->fontMetrics().horizontalAdvance(item->text());
...
#else
    QWidget::enterEvent(dynamic_cast<QEnterEvent*>(event));
#endif
